...
import { useState, useEffect, useRef } from 'react'
import { Send, Paperclip, LogOut, Stethoscope, Loader2, Activity, X } from 'lucide-react'
import { useRouter } from 'next/navigation'
import { createClient } from '@/lib/supabaseClient'

// --- Custom Chat Hook ---
// Modified to accept the pending image state so it can be sent with the text
function useCustomChat({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 1. Combine Text and Image
    let finalContent = input.trim()
    if (pendingImage) {
      // Append the image tag to the text description
      finalContent = `${finalContent} [Image: ${pendingImage}]`.trim()
    }
    ...
    setMessages(prev => [...prev, userMessage])
    setInput('')
    setPendingImage(null) // Clear the image preview after sending
    setIsLoading(true)
...
export default function ChatPage() {
  ...
  const [loadingAuth, setLoadingAuth] = useState(true)
  const [isUploading, setIsUploading] = useState(false)
  // New state to hold the image URL before sending
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // FIX: Instead of jamming it into the input text, set it to the preview state
    ...
    <div className="flex h-screen flex-col bg-slate-50">
      ...
      <header className="flex items-center justify-between bg-white px-6 py-4 shadow-sm border-b border-slate-200 z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            {/* RENAMED HERE */}
            <h1 className="font-bold text-slate-800 text-lg">Reply challenge of health care - team 25</h1>
            <div className="flex items-center gap-1.5">
              <span className="relative flex h-2.5 w-2.5">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-emerald-500"></span>
      ...
      <div className="bg-white px-4 py-4 border-t border-slate-200">
        <div className="mx-auto max-w-3xl">
          ...
          {pendingImage && (
            <div className="mb-3 flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 p-2 w-fit">
              <img src={pendingImage} alt="Preview" className="h-16 w-16 rounded object-cover" />
              <button 
                onClick={() => setPendingImage(null)}
                className="rounded-full bg-white p-1 text-slate-500 hover:bg-red-50 hover:text-red-500 shadow-sm border"
              >
                <X size={14} />
              </button>
            </div>
          ...
          <div className="flex items-end gap-3">
            ...
            <form onSubmit={handleSubmit} className="flex flex-1 gap-2">
              <input
                ...
                value={input}
                onChange={handleInputChange}
                placeholder={pendingImage ? "Add a description for the image..." : "Type your health query..."}
              />
              <button
                type="submit"
